...
import (
	...
	"github.com/mbhaskar98/orders-api/model"
...
func orderedIDKey(id uint64) string {
...
func (r *RedisRepo) Insert(ctx context.Context, order model.Order) error {
	...
	key := orderedIDKey(order.OrderID)
...
func (r *RedisRepo) FindAll(ctx context.Context, page FindAllPage) (FindResul, error) {
	...
	if err != nil {
		return FindResul{}, fmt.Errorf("failed to get order ids: %w", err)
	}

	if len(keys) == 0 {
		return FindResul{Orders: []model.Order{}}, nil
	}

	xs, err := r.Client.MGet(ctx, keys...).Result()
	if err != nil {
		return FindResul{}, fmt.Errorf("failed to get orders: %w", err)
	}
	...
	for i, x := range xs {
		...
		if err != nil {
			return FindResul{}, fmt.Errorf("failed to decode order json: %w", err)
		}
